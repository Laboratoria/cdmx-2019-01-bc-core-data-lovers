...
const print = (datosArr) => {
  ...
  datosArr.forEach(champ => {
    let championCard = `<div div id="${champ.id}" class="cuadro"><img class="imagen" src="${champ.splash}"> <div id="subtitulo"><h1 id="nombre">${champ.name}</h1> <h2 id"titulo"> ${champ.title}</h2></div></div>`;
...
const printCharacterSheet = (datosArr) => {
  ...
  datosArr.forEach(champ => {
    let championCard = `<div div id="${champ.id}" class="totalDescription"><img class="imagen" src="${champ.splash}"><h1 id= "nombre" >${champ.name}</h1> <p id="titulo">${champ.title}</p>
    <h2>${champ.blurb}</h2><div id="info"<p>Attack:${champ.attack}</p><p>Defense:${champ.defense}</p>
    <p>Magic:${champ.magic}</p><p>Difficulty:${champ.difficulty}</p>
    </div><div id="statsInfo"<h2>Stats</h2><p>Hp:${champ.hp}</p><p>Hp per level:${champ.hpperlevel}</p>
